      ...
      async function enrollSubscribersInSupremeLeagues() {
        ...
        for (const meta of metas) {
          for (const sub of subscriptions) {
            if (!subscriptionEntitles(sub, meta.cadence, meta)) {
              notEntitled += 1;
              continue;
            }
...
async function runDailyMaintenanceTasks() {
  // Correct Monthly Entry windows before generic expiry so a legacy 30-day date
  // cannot incorrectly expire a still-running monthly FPL cycle.
  const monthlySubscriptions = await reconcileMonthlySubscriptionWindows();
...
module.exports.claimDailyMaintenanceRun = claimDailyMaintenanceRun;
